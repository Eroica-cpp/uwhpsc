! $UWHPSC/codes/adaptive_quadrature/openmp1/adapquad_mod.f90
...
recursive subroutine adapquad(f,a,b,tol,intest,errest,level,fa,fb)
    ...
    integer :: thislevel, nextlevel, thread_num

    ! Unit 7 should be opened for writing out quadrature points in
    ! the main program.  Use this format statement for writes below:
701 format(2e20.10)

    ! Unit 8 should be opened for writing out intervals, level, thread_num in
    ! the main program.  Use this format statement for writes below:
801 format(2e20.10,i3,i3)
    ...
    thread_num = 0  ! for serial execution
    !$ thread_num = omp_get_thread_num()

    write(8,801) a,b,thislevel,thread_num
